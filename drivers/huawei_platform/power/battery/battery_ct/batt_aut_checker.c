...
static void check_key_func(struct work_struct *work)
{
    ...
    hwlog_info("Battery checker(NO:%d Group:%d) certification started.\n",
               drv_data->id_in_grp, drv_data->id_of_grp);
    if(bco->certification(drv_data->ic, &drv_data->key_res, &drv_data->result.key_status)) {
...
static void check_sn_func(struct work_struct *work)
{
    ...
    if(bco->prepare(drv_data->ic, RES_SN, &res)) {
    ...
    if(bco->get_batt_sn(drv_data->ic, &drv_data->sn_res, &drv_data->sn, &drv_data->sn_len)) {
...
static ssize_t batt_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    DEV_GET_DRVDATA(drv_data, dev, snprintf(buf, PAGE_SIZE, "Error driver data"));
    if(drv_data->bco.get_batt_type(drv_data->ic, &type, &type_len)) {
...
static ssize_t batt_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    DEV_GET_DRVDATA(drv_data, dev, snprintf(buf, PAGE_SIZE, "Error driver data"));
    ret = drv_data->bco.get_batt_sn(drv_data->ic, &drv_data->sn_res, &sn, &sn_len);
    if(ret) {
                                        ...
                                        char *buf)
{
    ...
    DEV_GET_DRVDATA(drv_data, dev, snprintf(buf, PAGE_SIZE, "Error driver data"));
    ret = drv_data->bco.get_batt_safe_info(drv_data->ic, BATT_MATCH_ABILITY, &temp);
    if(ret) {
                                  ...
                                  const char *buf, size_t count)
{
    ...
    ret = drv_data->bco.set_batt_safe_info(drv_data->ic, BATT_MATCH_ABILITY, &temp);
    if(ret) {
        return -1;
    }
    return count;
...
static int check_batt_ct_ops(batt_checker_data *drv_data)
{
    ...
    if(!drv_data->bco.get_batt_safe_info) {
        ret = BATTERY_DRIVER_FAIL;
...
static int battery_cycles_limit_init(struct platform_device *pdev)
{
    ...
    ret = drv_data->bco.get_batt_safe_info(drv_data->ic, BATT_MATCH_ABILITY,
