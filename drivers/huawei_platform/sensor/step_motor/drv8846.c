 ...
 * @return int SUCCESS or FAILURE.
 */
static void st_set_mode(const struct drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
static void ti_set_mode(const struct drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
static void st_start_motor(const struct drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
static void drv8846_param_set_default_val(struct drv_data *di)
{
	...
	drv8846_param_set_step_mode(di, FULL_STEP_RISING_ONLY);
